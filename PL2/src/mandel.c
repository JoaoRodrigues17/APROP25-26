...
result_t par_mandel(int num_threads){
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        // parallelize per line
        for (i=my_index*NPOINTS/n_threads; i<(my_index+1)*NPOINTS/n_threads; i++) {
            // #pragma omp parallel num_threads(num_threads)
            //parallelize per block
            for (j=0; j<NPOINTS; j++) {
                c.r = -2.0+2.5*(double)(i)/(double)(NPOINTS)+eps;
                c.i = 1.125*(double)(j)/(double)(NPOINTS)+eps;
                // #pragma omp parallel num_threads(num_threads)
                // parallelize each pixel
                #pragma omp critical
                testpoint(c);
            }
        }
...
result_t seq_mandel(int num_threads){
    int i, j;
    double area, error, eps  = 1.0e-5;
    for (i=0; i<NPOINTS; i++) {
        for (j=0; j<NPOINTS; j++) {
