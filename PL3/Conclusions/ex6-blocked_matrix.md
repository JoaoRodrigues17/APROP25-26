...
* a. Compare the execution time of the different approaches to parallelize the program  
* b. Compare the strategy used by both implementations for the execution of tasks by the threads
...
Which is a perfectly parallelizable algorithm that could be used in a real setting, for example, for image smoothing/blurring.

We will be using both parallel implementations of this algorithm (task per cell and task per block) developed in exercise 5 to perform this comparison and measuring the execution times with an average of 10 runs per algorithm to reduce the effect of possible fluctuations. The main function used for that purpose is as follows:
