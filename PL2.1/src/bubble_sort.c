...
#define ARR_LEN 100000		 // The length of the array
...
void sort_par(int x[], int size, int num_threads){
	...
	{
		changes = 0;
		#pragma omp parallel for reduction(+:changes) num_threads(num_threads)
		for (i = 1; i < ARR_LEN-1; i = i + 2){
			if (x[i] > x[i + 1])
			{
				int tmp;
				tmp = x[i];
